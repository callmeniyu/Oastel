...
export default async function BlogDetailsPage({ params }: Props) {
    ...
    try {
        ...
        if (response.success) {
            ...
            otherBlogs = response.data.filter((blog) => blog.slug !== slug).slice(0, 4)
        }
    } catch (error) {
    ...
    try {
        formattedDate = new Date(blogDetails.createdAt).toLocaleDateString("en-GB", {
            day: "numeric",
            month: "long",
            year: "numeric",
        })
    ...
    return (
        <div className="max-w-6xl mx-auto px-4 py-10 font-poppins">
            ...
            <div className="bg-primary_green text-white p-3 md:p-6 rounded-lg text-center space-y-3 mb-6">
                ...
                <p className="text-lg text-gray-100">{blogDetails.description || "No description available"}</p>
            ...
            {blogDetails.image && (
                <BlogImage src={blogDetails.image} alt={blogDetails.title || "Blog image"} title={blogDetails.title} />
            )}
            ...
            <article
                ...
                dangerouslySetInnerHTML={{ __html: blogDetails.content || "<p>No content available</p>" }}
            />
                ...
                <section className="mt-16">
                    <div className="flex items-center gap-2">
                        ...
                        <h2 className="text-2xl font-extrabold sm:font-bold text-primary_green mb-4 pt-2 min-w-max">
                            Also Read ({otherBlogs.length} blogs)
                        </h2>

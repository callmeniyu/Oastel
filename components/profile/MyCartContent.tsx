...
export default function MyCartContent() {
  ...
  useEffect(() => {
    const fetchCartItems = async () => {
      ...
      try {
        setLoading(true);
        const userCartItems = await cartApi.getCartWithDetails(
          session.user.email
        );

        // Transform API cart items to component format
        const transformedItems: CartItemDisplay[] = userCartItems.map(
          (item: CartItem, index: number) => {
            return {
              id: item.id || `cart-${index}`,
              type: item.packageType,
              title:
                item.packageDetails?.title || `${item.packageType} Package`,
  ...
  const removeFromCart = async (id: string, index: number) => {
    ...
    try {
      const success = await cartApi.removeFromCart(session.user.email, index);
      if (success) {
